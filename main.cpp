...
LRESULT CALLBACK WndProc(HWND hWnd,UINT msg,WPARAM wParam,LPARAM lParam)
{
	...
	{
	case WM_CREATE:
		hEidt=CreateWindowEx(WS_EX_CLIENTEDGE,TEXT("EDIT"),TEXT("C:\\Program Files"),WS_VISIBLE|WS_CHILD,10,10,512,30,hWnd,(HMENU)100,((LPCREATESTRUCT)lParam)->hInstance,0);
		hStatic=CreateWindow(TEXT("STATIC"),0,WS_VISIBLE|WS_CHILD,10,50,512,30,hWnd,0,((LPCREATESTRUCT)lParam)->hInstance,0);
		CreateWindow(TEXT("BUTTON"),TEXT("「\"\"」付加"),WS_VISIBLE|WS_CHILD,512+20,10,128,30,hWnd,(HMENU)101,((LPCREATESTRUCT)lParam)->hInstance,0);
		break;
	case WM_COMMAND:
		if(LOWORD(wParam)==101)
		{
			...
			PathQuoteSpaces(szFilePath);// パスにスペースが含まれて居る場合のみダブルクォーテーションで囲む
